...
- **Etapa atual:** 2 — Gerador de dados sintéticos ✅ CONCLUÍDA
- **Última sessão:** 10/07/2026 — Etapa 2 executada na branch `etapa-2-dados`: PR #1 (Etapa 1) mergeado na `main`; `scripts/generate_synthetic.py` gera 12 meses de dados via `psycopg2` + `execute_values` (carga em lote): 6 fornecedores, 24 produtos, 212 notas fiscais, 734 linhas de nota, 10 receitas, 27 receita_ingredientes, 3640 registros de venda. Modelo de preço multiplicativo (inflação × sazonalidade por categoria × fator de fornecedor × ruído log-normal) e demanda diária via Poisson (fator dia-da-semana + crescimento mensal). Corrigido bug de sazonalidade que usava índice relativo do mês em vez do mês-calendário real da data. Dados validados por amostragem (tendência de preço do Tomate confere com sazonalidade esperada, zero linhas inválidas)
- **Próximo passo:** Etapa 3 — app base com CRUD de notas fiscais (Streamlit multipage conectado ao Supabase)
- **Decisões tomadas:** extração por foto com VLM local via Ollama como padrão, Claude API como alternativa plugável; app mobile via navegador (sem app nativo); MVP para um único estabelecimento; schema `restaurant` dedicado no Postgres; `unidade_medida` e `fonte` via `CHECK` constraint (não tabela de lookup) nesta fase; scripts de seed usam `psycopg2` direto (não `supabase-py`) para carga em lote
