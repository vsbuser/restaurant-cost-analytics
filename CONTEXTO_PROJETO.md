...
- **Etapa atual:** 5 — Dashboard analítico ✅ CONCLUÍDA
- **Última sessão:** 13/07/2026 — Etapa 5 executada na branch `etapa-5-dashboard`: PR #4 (Etapa 4) mergeado na `main`; `sql/views.sql` ganha `vw_preco_medio_mensal` e `vw_variacao_preco` (com alerta se subir >10%); nova página `app/pages/5_Dashboard.py` com 4 KPIs (food cost global 25%, gasto total, receita total, alertas ativos) e 5 gráficos Plotly — cada um com a cor escolhida pelo job dos dados (status para o semáforo de food cost, emphasis para o alerta de variação, categórica para a evolução de preço por insumo, sequencial para os rankings de fornecedor/categoria), seguindo o método da skill `dataviz`. Testado ponta a ponta com Playwright, sem erros de console. Na sequência, redesign visual completo (fora da numeração de etapas): 3 direções propostas (Ledger/Control Tower/Kitchen Fresh), escolhida **Kitchen Fresh** — tema verde aplicado via `.streamlit/config.toml` + `app/components/theme.py` (cards de KPI, sidebar verde-escuro, tipografia Poppins, cores dos gráficos do Dashboard atualizadas para a nova marca) em todas as páginas
- **Próximo passo:** Etapa 6 — extração de notas por foto com IA local (Ollama/Qwen2.5-VL, backend FastAPI, `st.camera_input()`)
- **Decisões tomadas:** extração por foto com VLM local via Ollama como padrão, Claude API como alternativa plugável; app mobile via navegador (sem app nativo); MVP para um único estabelecimento; schema `restaurant` dedicado no Postgres; `unidade_medida` e `fonte` via `CHECK` constraint (não tabela de lookup) nesta fase; scripts de seed e app usam `psycopg2` direto (não `supabase-py`) — schema `restaurant` não é exposto pela API REST do Supabase por padrão; formulário de nova nota/receita usa lista incremental via `session_state`, não `data_editor`, para ser usável no celular; pool de conexões faz health-check antes de entregar conexão (pooler do Supabase mata conexões ociosas); gráficos usam cor por job dos dados (status/categórica/emphasis/sequencial), nunca cor genérica; identidade visual "Kitchen Fresh" (verde `#1e8f5e`, cantos arredondados, Poppins nos títulos), nome do app `RestaurApp`
