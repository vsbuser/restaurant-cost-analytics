...
- **Etapa atual:** 6 — Extração de notas por foto com IA local 🚧 EM ANDAMENTO
- **Última sessão:** 13/07/2026 — Etapa 5 concluída e mergeada (PR #5); iniciada a Etapa 6 na branch `etapa-6-extracao-foto`: Ollama instalado localmente + modelo `minicpm-v` baixado (~5.5GB); pipeline completo escrito — `backend/preprocessing.py` (OpenCV: correção de perspectiva, contraste, resize), `backend/vision.py` (chama Ollama), `backend/main.py` (FastAPI `POST /extract`), `app/services/extractor.py` (abstração com dois provedores: `ollama` local ou `claude` via SDK Anthropic, trocável por `.env`), nova aba "Tirar foto" em `app/pages/3_Notas.py`. Testado via `curl` direto no backend com uma nota fiscal sintética: extração correta após corrigir um bug real (preços vinham como texto formatado tipo "R$ 6,20" em vez de número — `format: "json"` do Ollama garante JSON válido mas não os tipos do schema; corrigido com sanitização em `vision.py`). Ainda **não testado** o fluxo completo pela UI do Streamlit
- **Próximo passo:** retomar a Etapa 6 — testar a aba "Tirar foto" ponta a ponta no navegador; decidir sobre upload da foto original ao Supabase Storage (ainda não implementado); considerar testar `qwen2.5-vl` para comparação
- **Decisões tomadas:** extração por foto com VLM local via Ollama como padrão (modelo escolhido para teste: `minicpm-v`), Claude API como alternativa plugável; app mobile via navegador (sem app nativo); MVP para um único estabelecimento; schema `restaurant` dedicado no Postgres; `unidade_medida` e `fonte` via `CHECK` constraint (não tabela de lookup) nesta fase; scripts de seed e app usam `psycopg2` direto (não `supabase-py`) — schema `restaurant` não é exposto pela API REST do Supabase por padrão; formulário de nova nota/receita usa lista incremental via `session_state`, não `data_editor`, para ser usável no celular; pool de conexões faz health-check antes de entregar conexão (pooler do Supabase mata conexões ociosas); gráficos usam cor por job dos dados (status/categórica/emphasis/sequencial), nunca cor genérica; identidade visual "Kitchen Fresh" (verde `#1e8f5e`, cantos arredondados, Poppins nos títulos), nome do app `RestaurApp`; extração local sanitiza números na origem (backend), não confia no tipo devolvido pelo modelo
- **Decisões pendentes:** nome definitivo da app; `qwen2.5-vl` vs `minicpm-v` (o segundo já testado e funcionando; comparação formal ainda pendente); upload da foto original ao Supabase Storage
