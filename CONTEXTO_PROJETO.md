...
- **Etapa atual:** 4 — Fichas técnicas e food cost ✅ CONCLUÍDA
- **Última sessão:** 13/07/2026 — Etapa 4 executada na branch `etapa-4-fichas`: PR #3 (Etapa 3) mergeado na `main`; `sql/views.sql` com 3 views encadeadas (`vw_ultimo_preco_produto` via window function, `vw_custo_receita`, `vw_food_cost` com semáforo verde/amarelo/vermelho), aplicadas e validadas no Supabase (os 10 pratos cobrem as três faixas, ex: hambúrguer de picanha 39,1% vermelho, salada tropical 5,2% verde). Nova página `app/pages/4_Fichas.py`: lista de fichas com semáforo + detalhe expansível dos ingredientes, formulário de nova receita com editor incremental de ingredientes. **Bug real encontrado e corrigido:** `app/services/database.py` devolvia conexões mortas ao pool (o pooler do Supabase mata conexões ociosas), travando a app inteira — corrigido com ping de saúde antes de entregar a conexão e descarte em caso de falha (detalhe em `docs/decisoes.md`). Testado ponta a ponta (Playwright + verificação direta no banco)
- **Próximo passo:** Etapa 5 — dashboard analítico (KPIs de food cost, evolução de preços, comparativo de fornecedores)
- **Decisões tomadas:** extração por foto com VLM local via Ollama como padrão, Claude API como alternativa plugável; app mobile via navegador (sem app nativo); MVP para um único estabelecimento; schema `restaurant` dedicado no Postgres; `unidade_medida` e `fonte` via `CHECK` constraint (não tabela de lookup) nesta fase; scripts de seed e app usam `psycopg2` direto (não `supabase-py`) — schema `restaurant` não é exposto pela API REST do Supabase por padrão; formulário de nova nota/receita usa lista incremental via `session_state`, não `data_editor`, para ser usável no celular; pool de conexões faz health-check antes de entregar conexão (pooler do Supabase mata conexões ociosas)
